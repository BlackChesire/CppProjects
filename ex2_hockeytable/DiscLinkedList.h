...
#ifndef INC_32_EX3_DISCLINKEDLIST_H
#define INC_32_EX3_DISCLINKEDLIST_H
#include "Disc.h"
...
public:
	struct node
	{
		Disc disc;
		node *next;
		node(const Disc &disc, node *next) : disc(disc), next(next) {}
	};
	DiscLinkedList();
	//rule of 3
	...
	~DiscLinkedList();				 //dtor
	void add_disc(const Disc &disc); //adds disc to the list
...
#endif //INC_32_EX3_DISCLINKEDLIST_H
