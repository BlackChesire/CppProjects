...
: Disc(x1, y1,vx, vy,radius),lives_count(lives_count),is_shrinked(false),discType(Shrinking) {}

ShrinkingDisc::ShrinkingDisc(const ShrinkingDisc &other)
: Disc(other),lives_count(other.getLivesCount()),is_shrinked(other.isShrinked()),discType(other.getDiscType()) {
...
bool ShrinkingDisc::isShrinked() const {
    return is_shrinked;
}
