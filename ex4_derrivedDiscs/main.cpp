...
int main() {
    ...
    while (cin>>cmd || !cin.eof()){
        if (cmd=="disc"){
            ...
            hockeyTable.discLinkedlist.add_disc(new Disc(x, y, vx, vy, radius));
        } else if (cmd == "wall"){
            cin>>x1>>y1>>x2>>y2;
            if(cin.fail()){
                cerr<<"Error: illegal input."<<endl;
            ...
            hockeyTable.check_stepover(Disc(x, y, vx, vy, radius));
            hockeyTable.discLinkedlist.add_disc(new ExplodingDisc(x,y,vx,vy,radius,radius*radius,0));
        }
            ...
            hockeyTable.check_stepover(Disc(x, y, vx, vy, radius));
            hockeyTable.discLinkedlist.add_disc(new ShrinkingDisc(x,y,vx,vy,radius,0));
        }
        else{
            cerr<<"Error: illegal input."<<endl;
            return 0;
